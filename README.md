...
RTS-Tender выделен как отдельный стратегический контур. Публичный RTS-market теперь запускается в основном live-слое и пишет отдельные health-строки по endpoint-ам: Rosatom, общий RTS-market, Симферополь и Ялта. Безопасные ограничения сохраняются: капча не обходится, закрытые разделы не автоматизируются, а кабинетный/API-режим остается следующим отдельным шагом после проверки ЛК.
