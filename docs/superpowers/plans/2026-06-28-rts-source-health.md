...
- [ ] Add a failing test where one RTS endpoint is blocked and another returns tenders.
- [ ] Verify the test fails because `RtsPublicSource` has no `fetch_with_report`.
- [ ] Implement endpoint-level reporting and exception classification.
- [ ] Make `fetch_keywords()` call `fetch_with_report()` and preserve old all-failed behavior.
- [ ] Verify `tests/test_rts_source.py` passes.
- [ ] Commit as `Add RTS endpoint health report`.
...
- [ ] Add a failing test proving `CompositeSource` uses a normal source's `fetch_with_report`.
- [ ] Verify the test fails because only nested `CompositeSource` gets special handling.
- [ ] Implement a small helper that detects and calls report-aware sources.
- [ ] Verify `tests/test_composite_source.py` passes.
- [ ] Commit as `Use source-level health reports in composite`.
...
- [ ] Update the default source test to expect `RtsPublicSource` in the main layer.
- [ ] Verify the test fails because RTS is still fallback-only.
- [ ] Move RTS into the main live layer and remove the extra fallback wrapper.
- [ ] Verify CLI tests pass.
- [ ] Commit as `Run RTS in main tender source layer`.
...
- [ ] Document that RTS now runs in the main source layer with endpoint health.
