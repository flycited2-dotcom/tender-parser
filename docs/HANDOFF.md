...
- Поисковик RTS по всем ЭТП (`www.rts-tender.ru/poisk/search?id=...`): прямые HTTP-запросы из текущей сети невозможны — SSL handshake к `www.rts-tender.ru` таймаутится и напрямую, и через внешний канал (та же картина, что с публичным RTS-market). Поэтому сбор пойдет через Chrome пользователя: автосбор теперь замечает вкладки `/poisk` и сохраняет их отрендеренную вёрстку в `diagnostics/rts_poisk_*.html` (один раз на содержимое, до 1 снапшота на страницу). Следующий шаг: пользователь открывает свой сохраненный поиск при запущенном автосборе, по снапшоту пишется точный парсер результатов поиска, и накопитель начинает принимать строки поиска так же, как строки кабинета.
