...
- Следующая целевая архитектура: добавить ЕИС/`zakupki.gov.ru` как главный широкий источник для 44-ФЗ/223-ФЗ, а отдельные ЭТП использовать как дополнительные каналы для коммерческих, малых и региональных закупок.
...
1. Сделать spike по ЕИС/`zakupki.gov.ru`: проверить текущий доступный канал в 2026 году - публичный поиск/экспорт, официальный XML/open-data канал или кабинетный/интеграционный доступ. Не опираться на старые FTP-схемы без live-проверки.
2. Добавить `EisZakupkiSource`: построение запросов по регионам/ключам, разбор списка, нормализация суммы/дедлайна/заказчика/ссылки, дедупликация с текущими источниками.
3. Добавить source health report в консоль и, желательно, в JSON: статус источника, ошибка или причина пропуска, сколько карточек найдено и сколько прошло фильтр.
4. Получить в ЛК ЕАТ токен и код внешней системы, затем проверить `EatIntegrationSource` live-запуском.
5. Углубить Торги82: найти корректный GWT/searchServlet payload для ключевых слов и пагинации, потому что простой endpoint сейчас дает только последние 20 процедур.
6. Добавить B2B-Center через публичный HTML или личный кабинет; публичная страница сейчас может показывать anti-bot/rate-limit.
7. После этого смотреть Фабрикант, ОТС и OnlineContract.
8. Для CRM лучше сначала читать `exports/latest.json`; сейчас там подходящие и `review`-кандидаты.
