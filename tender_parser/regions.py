...
# Город хранится в Excel отдельно от субъекта РФ. Список ограничен целевыми
# регионами, чтобы случайное упоминание одноимённого города из другого субъекта
# не превращалось в уверенный фильтр. Более длинные названия идут раньше
# коротких: «Новая Каховка» должна победить «Каховку».
CITY_VARIANTS: dict[str, list[str]] = {
    "Новая Каховка": ["новая каховк"],
    "Каменка-Днепровская": ["каменка-днепровск"],
    "Красноперекопск": ["красноперекопск"],
    "Старый Крым": ["старый крым"],
    "Голая Пристань": ["голая пристан"],
    "Симферополь": ["симферопол"],
    "Севастополь": ["севастопол"],
    "Евпатория": ["евпатори"],
    "Феодосия": ["феодоси"],
    "Бахчисарай": ["бахчисара"],
    "Джанкой": ["джанко"],
    "Белогорск": ["белогорск"],
    "Щёлкино": ["щелкино"],
    "Мелитополь": ["мелитопол"],
    "Бердянск": ["бердянск"],
    "Энергодар": ["энергодар"],
    "Днепрорудное": ["днепрорудн"],
    "Геническ": ["геническ"],
    "Скадовск": ["скадовск"],
    "Каховка": ["каховк"],
    "Алёшки": ["алешк"],
...
def detect_city(text: str | None) -> str | None:
    """Return a target-region city when it is explicitly present in text."""
